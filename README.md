![header](https://capsule-render.vercel.app/api?type=waving&color=6A0DAD&height=200&section=header&text=&fontSize=50&fontColor=ffffff&animation=fadeIn)

# Olá, eu sou a Daiane 👋

21 anos · Estudante de Sistemas da Informação · Explorando o mundo do desenvolvimento

> Estou no início da minha jornada em TI e descobrindo quais áreas mais me interessam. Atualmente, tenho me dedicado ao desenvolvimento front-end :)

---

### 🛠️ Tecnologias que estou aprendendo

![HTML](https://img.shields.io/badge/HTML-6A0DAD?style=flat&logo=html5&logoColor=white)
![CSS](https://img.shields.io/badge/CSS-7B2FBE?style=flat&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-9B59B6?style=flat&logo=javascript&logoColor=white)
![Git](https://img.shields.io/badge/Git-8E44AD?style=flat&logo=git&logoColor=white)
![REST API](https://img.shields.io/badge/REST_API-A569BD?style=flat&logo=fastapi&logoColor=white)

---

### 🚀 Projeto em destaque

**PRISMA — Plataforma de Inglês**
Plataforma web com múltiplas páginas, responsividade mobile-first, menu hambúrguer,
sistema de autenticação, modais de pagamento e integração com API.
Desenvolvida com HTML, CSS e JavaScript puro.

---

### 📊 Minhas estatísticas

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=anemts&layout=compact&theme=tokyonight&title_color=9b59b6&border_color=6A0DAD&locale=pt-br)

---
### 📌 Atualmente estou

- 📚 Estudando Tecnologia da Informação
- 🔍 Explorando diferentes áreas da tecnologia
- 🚀 Construindo projetos para aprender na prática

---

![footer](https://capsule-render.vercel.app/api?type=waving&color=6A0DAD&height=120&section=footer)
